...
`,yt={sensor:"sensors",binary_sensor:"binary_sensors",button:"buttons",switch:"switches",number:"numbers",select:"selects"};function wt(t){const e=t.indexOf(".");if(e<0)return;const i=t.substring(e+1);for(const t of bt)if(i.endsWith(t)){const e=i.substring(0,i.length-t.length);if(""===e||e.endsWith("_"))return t}}function xt(t,e){if(t.entities)for(const[i,s]of Object.entries(t.entities))if(s.device_id===e)return i}function At(t,e){if(!e)return;const i=t.states[e]?.state;return"unavailable"!==i&&"unknown"!==i?i:void 0}function Et(t,e){const i=At(t,e);if(void 0===i)return;const s=Number(i);return isNaN(s)?void 0:s}function St(t,e){if(!e)return!1;const i=t.states[e]?.state;return"on"===i||"connected"===i||"true"===i||"True"===i}function Ct(t,e){const i=At(t,e);if(i)try{const t=new Date(i);return isNaN(t.getTime())?i:t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}catch{return i}}console.info("%c PETLIBRO-CARD %c v0.3.0 ","color: white; background: #3498db; font-weight: bold;","color: #3498db; background: white; font-weight: bold;"),window.customCards=window.customCards||[],window.customCards.push({type:vt,name:"Petlibro Card",description:"Auto-detecting card for PetLibro feeders, fountains, and litter boxes",preview:!0});let kt=class extends at{static{this.styles=$t}static async getConfigElement(){return await Promise.resolve().then(function(){return Ot}),document.createElement(mt)}static getStubConfig(){return{device_id:""}}setConfig(t){if(!t.device_id&&!t.entity)throw new Error("Please select a PetLibro device");this._config={show_controls:!0,...t}}getCardSize(){return 4}shouldUpdate(t){if(t.has("_config"))return!0;if(!t.has("hass")||!this._entities)return!0;const e=t.get("hass");if(!e)return!0;return this._getAllEntityIds().some(t=>e.states[t]!==this.hass.states[t])}willUpdate(t){var e,i;if(this.hass&&this._config&&(t.has("hass")||t.has("_config"))){let t;this._config.device_id?t=this._config.device_id:this._config.entity&&(e=this.hass,i=this._config.entity,t=e.entities?.[i]?.device_id??void 0),t&&t!==this._deviceId&&(this._deviceId=t,this._entities=function(t,e){const i={sensors:{},binary_sensors:{},buttons:{},switches:{},numbers:{},selects:{}};if(!t.entities)return i;for(const[s,o]of Object.entries(t.entities)){if(o.device_id!==e)continue;const t=s.substring(0,s.indexOf(".")),n=yt[t];if(!n)continue;const r=wt(s);r&&(i[n][r]=s)}return i}(this.hass,t),this._deviceType=function(t){const e=new Set([...Object.keys(t.sensors),...Object.keys(t.binary_sensors),...Object.keys(t.buttons),...Object.keys(t.switches)]);return ft.some(t=>e.has(t))?"litter_box":pt.some(t=>e.has(t))?"feeder":gt.some(t=>e.has(t))?"fountain":"feeder"}(this._entities))}}render(){if(!this._config||!this.hass)return B``;if(!this._deviceId||!this._entities||!this._deviceType)return B`
        <ha-card>
    ...
    `}_renderHeader(){const t=this._config.name||(e=this.hass,i=this._deviceId,e.devices?.[i]?.name??void 0)||"PetLibro Device";var e,i;const s=this._config.entity||xt(this.hass,this._deviceId)||"",o=function(t,e,i){const s=t.states[e]?.attributes?.entity_picture;return s||(t.devices?.[i]?.configuration_url??void 0)}(this.hass,s,this._deviceId),n=St(this.hass,this._entities.binary_sensors.online??this._entities.binary_sensors.wi_fi),r=this.hass.devices?.[this._deviceId]?.model;return B`
    ...
    `}_renderDeviceContent(){if(!this._entities)return F;switch(this._deviceType){case"feeder":return function(t,e,i,s){const o=Et(t,e.sensors.electric_quantity),n=St(t,e.binary_sensors.food_low),r=At(t,e.sensors.today_feeding_quantity_weight),a=At(t,e.sensors.today_feeding_times),c=Ct(t,e.sensors.last_feed_time),d=Ct(t,e.sensors.next_feed_time),l=At(t,e.sensors.next_feed_quantity_weight),h=At(t,e.sensors.feeding_plan_state),u=t.states[e.sensors.today_feeding_quantity_weight??""]?.attributes?.unit_of_measurement??"g",_=t.states[e.sensors.next_feed_quantity_weight??""]?.attributes?.unit_of_measurement??"g",v=St(t,e.binary_sensors.light_switch),m=At(t,e.sensors.temperature),p=At(t,e.selects.plate_position),g=At(t,e.sensors.next_feeding_time),f=At(t,e.sensors.next_feeding_end_time),b=At(t,e.sensors.next_feeding_day),$=Et(t,e.sensors.remaining_cleaning_days);return B`
    <div class="metrics-grid">
      ${void 0!==o?B`
        <div class="metric-item">
          <ha-icon class="metric-icon" icon="${y=o,y>=90?"mdi:battery":y>=70?"mdi:battery-70":y>=50?"mdi:battery-50":y>=30?"mdi:battery-30":y>=10?"mdi:battery-10":"mdi:battery-alert"}"></ha-icon>
      ...
      ${void 0!==r?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${r} ${u}${a?` (${a}x)`:""}</div>
          </div>
        </div>
      `:F}
      ...
      ${d?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${d}${l?` (${l} ${_})`:""}</div>
          </div>
        </div>
      `:F}
  ...
  `;var y}(this.hass,this._entities,t=>this._handleButtonPress(t),t=>this._handleSwitchToggle(t));case"fountain":return function(t,e,i){const s=Et(t,e.sensors.electric_quantity),o=Et(t,e.sensors.weight_percent),n=St(t,e.binary_sensors.water_low),r=At(t,e.sensors.remaining_water),a=t.states[e.sensors.remaining_water??""]?.attributes?.unit_of_measurement??"mL",c=At(t,e.sensors.today_drinking_amount),d=t.states[e.sensors.today_drinking_amount??""]?.attributes?.unit_of_measurement??"mL",l=At(t,e.sensors.remaining_filter_days),h=At(t,e.sensors.remaining_cleaning_days),u=St(t,e.binary_sensors.light_switch),_=void 0!==o?o<=10?"error":o<=25?"warning":"":"";return B`
    <div class="metrics-grid">
      ${void 0!==s?B`
        <div class="metric-item">
