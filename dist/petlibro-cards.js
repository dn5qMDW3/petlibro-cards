...
`,$t={sensor:"sensors",binary_sensor:"binary_sensors",button:"buttons",switch:"switches",number:"numbers",select:"selects"};function wt(t){const e=t.indexOf(".");if(e<0)return;const i=t.substring(e+1);for(const t of bt)if(i.endsWith(t)){const e=i.substring(0,i.length-t.length);if(""===e||e.endsWith("_"))return t}}function xt(t,e){if(!e)return;const i=t.states[e]?.state;return"unavailable"!==i&&"unknown"!==i?i:void 0}function At(t,e){const i=xt(t,e);if(void 0===i)return;const s=Number(i);return isNaN(s)?void 0:s}function Et(t,e){if(!e)return!1;const i=t.states[e]?.state;return"on"===i||"true"===i||"True"===i}function St(t,e){const i=xt(t,e);if(i)try{const t=new Date(i);return isNaN(t.getTime())?i:t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}catch{return i}}console.info("%c PETLIBRO-CARD %c v0.1.0 ","color: white; background: #3498db; font-weight: bold;","color: #3498db; background: white; font-weight: bold;"),window.customCards=window.customCards||[],window.customCards.push({type:vt,name:"Petlibro Card",description:"Auto-detecting card for PetLibro feeders, fountains, and litter boxes",preview:!0});let Ct=class extends at{static{this.styles=yt}static async getConfigElement(){return await Promise.resolve().then(function(){return Pt}),document.createElement(mt)}static getStubConfig(){return{entity:""}}setConfig(t){if(!t.entity)throw new Error("Please define an entity");this._config={show_controls:!0,...t}}getCardSize(){return 4}shouldUpdate(t){if(t.has("_config"))return!0;if(!t.has("hass")||!this._entities)return!0;const e=t.get("hass");if(!e)return!0;return this._getAllEntityIds().some(t=>e.states[t]!==this.hass.states[t])}willUpdate(t){var e,i;if(this.hass&&this._config?.entity&&(t.has("hass")||t.has("_config"))){const t=(e=this.hass,i=this._config.entity,e.entities?.[i]?.device_id??void 0);t&&t!==this._deviceId&&(this._deviceId=t,this._entities=function(t,e){const i={sensors:{},binary_sensors:{},buttons:{},switches:{},numbers:{},selects:{}};if(!t.entities)return i;for(const[s,n]of Object.entries(t.entities)){if(n.device_id!==e)continue;const t=s.substring(0,s.indexOf(".")),o=$t[t];if(!o)continue;const r=wt(s);r&&(i[o][r]=s)}return i}(this.hass,t),this._deviceType=function(t){const e=new Set([...Object.keys(t.sensors),...Object.keys(t.binary_sensors),...Object.keys(t.buttons),...Object.keys(t.switches)]);return gt.some(t=>e.has(t))?"litter_box":pt.some(t=>e.has(t))?"feeder":ft.some(t=>e.has(t))?"fountain":"feeder"}(this._entities))}}render(){if(!this._config||!this.hass)return B``;if(!this._deviceId||!this._entities||!this._deviceType)return B`
        <ha-card>
      ...
      `;const t=this.hass.states[this._config.entity]?.state;return"unavailable"===t?B`
        <ha-card>
          ${this._renderHeader()}
          <div class="unavailable">
    ...
    `}_renderHeader(){const t=this._config.name||(e=this.hass,i=this._deviceId,e.devices?.[i]?.name??void 0)||"PetLibro Device";var e,i;const s=function(t,e,i){const s=t.states[e]?.attributes?.entity_picture;return s||(t.devices?.[i]?.configuration_url??void 0)}(this.hass,this._config.entity,this._deviceId),n=Et(this.hass,this._entities.binary_sensors.online),o=this.hass.devices?.[this._deviceId]?.model;return B`
      <div class="card-header">
        ${s?B`<img class="device-image" src="${s}" alt="${t}" />`:B`
        ...
        <div class="header-info">
          ...
          ${o?B`<div class="device-model">${o}</div>`:F}
        </div>
    ...
    `}_renderDeviceContent(){if(!this._entities)return F;switch(this._deviceType){case"feeder":return function(t,e,i){const s=At(t,e.sensors.electric_quantity),n=Et(t,e.binary_sensors.food_low),o=xt(t,e.sensors.today_feeding_quantity_weight),r=xt(t,e.sensors.today_feeding_times),a=St(t,e.sensors.last_feed_time),c=St(t,e.sensors.next_feed_time),d=xt(t,e.sensors.next_feed_quantity_weight),l=xt(t,e.sensors.feeding_plan_state),h=t.states[e.sensors.today_feeding_quantity_weight??""]?.attributes?.unit_of_measurement??"g",u=t.states[e.sensors.next_feed_quantity_weight??""]?.attributes?.unit_of_measurement??"g",_=Et(t,e.binary_sensors.light_switch);return B`
      ...
      ${void 0!==o?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${o} ${h}${r?` (${r}x)`:""}</div>
          </div>
        </div>
      `:F}
  ...
  `;var v}(this.hass,this._entities,t=>this._handleButtonPress(t));case"fountain":return function(t,e,i){const s=At(t,e.sensors.electric_quantity),n=At(t,e.sensors.weight_percent),o=Et(t,e.binary_sensors.water_low),r=xt(t,e.sensors.remaining_water),a=t.states[e.sensors.remaining_water??""]?.attributes?.unit_of_measurement??"mL",c=xt(t,e.sensors.today_drinking_amount),d=t.states[e.sensors.today_drinking_amount??""]?.attributes?.unit_of_measurement??"mL",l=xt(t,e.sensors.remaining_filter_days),h=xt(t,e.sensors.remaining_cleaning_days),u=Et(t,e.binary_sensors.light_switch),_=void 0!==n?n<=10?"error":n<=25?"warning":"":"";return B`
    <div class="metrics-grid">
      ${void 0!==s?B`
        <div class="metric-item">
          <ha-icon class="metric-icon" icon="${v=s,v>=90?"mdi:battery":v>=70?"mdi:battery-70":v>=50?"mdi:battery-50":v>=30?"mdi:battery-30":v>=10?"mdi:battery-10":"mdi:battery-alert"}"></ha-icon>
      ...
      <div class="metric-item ${o?"alert":""}">
        <ha-icon class="metric-icon" icon="${o?"mdi:water-off":"mdi:water-percent"}"></ha-icon>
        <div class="metric-content">
          <div class="metric-label">Water Level</div>
          <div class="metric-value">${void 0!==n?`${Math.round(n)}%`:o?"Low":"OK"}</div>
          ${void 0!==n?B`
            <div class="gauge-bar">
              <div class="gauge-fill ${_}" style="width: ${Math.min(100,n)}%"></div>
            </div>
          `:F}
        </div>
      </div>
  ...
  `;var v}(this.hass,this._entities,t=>this._handleButtonPress(t));case"litter_box":return function(t,e,i,s){const n=At(t,e.sensors.electric_quantity),o=At(t,e.sensors.weight_percent),r=Et(t,e.binary_sensors.rubbish_full_state),a=xt(t,e.sensors.running_state),c=xt(t,e.sensors.remaining_cleaning_days),d=xt(t,e.sensors.deodorization_mode),l=Et(t,e.switches.light_switch),h=Et(t,e.switches.sound_switch),u=void 0!==o?o<=15?"error":o<=30?"warning":"":"";return B`
    <div class="metrics-grid">
      ${void 0!==n?B`
        <div class="metric-item">
          <ha-icon class="metric-icon" icon="${_=n,_>=90?"mdi:battery":_>=70?"mdi:battery-70":_>=50?"mdi:battery-50":_>=30?"mdi:battery-30":_>=10?"mdi:battery-10":"mdi:battery-alert"}"></ha-icon>
          <div class="metric-content">
            <div class="metric-label">Battery</div>
  ...
  `}setConfig(t){this._config=t}render(){return this.hass&&this._config?B`
    ...
    `:B``}_entityChanged(t){const e=t.detail.value;e&&this._updateConfig({...this._config,entity:e})}_nameChanged(t){const e=t.target.value||void 0;this._updateConfig({...this._config,name:e})}_showControlsChanged(t){const e=t.target.checked;this._updateConfig({...this._config,show_controls:e})}_updateConfig(t){this._config=t;const e=new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0});this.dispatchEvent(e)}};t([ut({attribute:!1})],kt.prototype,"hass",void 0),t([_t()],kt.prototype,"_config",void 0),kt=t([dt(mt)],kt);var Pt=Object.freeze({__proto__:null,get PetlibroCardEditor(){return kt}});export{Ct as PetlibroCard};
