...
`,wt={sensor:"sensors",binary_sensor:"binary_sensors",button:"buttons",switch:"switches",number:"numbers",select:"selects"};function xt(t){const e=t.indexOf(".");if(e<0)return;const i=t.substring(e+1);for(const t of yt)if(i.endsWith(t)){const e=i.substring(0,i.length-t.length);if(""===e||e.endsWith("_"))return bt[t]??t}}function At(t,e){if(t.entities)for(const[i,s]of Object.entries(t.entities))if(s.device_id===e)return i}function kt(t,e){if(!e)return;const i=t.states[e]?.state;return"unavailable"!==i&&"unknown"!==i?i:void 0}function Et(t,e){const i=kt(t,e);if(void 0===i)return;const s=Number(i);return isNaN(s)?void 0:s}function St(t,e){if(!e)return!1;const i=t.states[e]?.state;return"on"===i||"connected"===i||"true"===i||"True"===i}function Ct(t,e){const i=kt(t,e);if(i)try{const t=new Date(i);return isNaN(t.getTime())?i:t.toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}catch{return i}}console.info("%c PETLIBRO-CARD %c v0.4.0 ","color: white; background: #3498db; font-weight: bold;","color: #3498db; background: white; font-weight: bold;"),window.customCards=window.customCards||[],window.customCards.push({type:vt,name:"Petlibro Card",description:"Auto-detecting card for PetLibro feeders, fountains, and litter boxes",preview:!0});let Pt=class extends at{static{this.styles=$t}static async getConfigElement(){return await Promise.resolve().then(function(){return Tt}),document.createElement(mt)}static getStubConfig(){return{device_id:""}}setConfig(t){if(!t.device_id&&!t.entity)throw new Error("Please select a PetLibro device");this._config={show_controls:!0,...t}}getCardSize(){return 4}shouldUpdate(t){if(t.has("_config"))return!0;if(!t.has("hass")||!this._entities)return!0;const e=t.get("hass");if(!e)return!0;return this._getAllEntityIds().some(t=>e.states[t]!==this.hass.states[t])}willUpdate(t){var e,i;if(this.hass&&this._config&&(t.has("hass")||t.has("_config"))){let t;this._config.device_id?t=this._config.device_id:this._config.entity&&(e=this.hass,i=this._config.entity,t=e.entities?.[i]?.device_id??void 0),t&&t!==this._deviceId&&(this._deviceId=t,this._entities=function(t,e){const i={sensors:{},binary_sensors:{},buttons:{},switches:{},numbers:{},selects:{}};if(!t.entities)return i;for(const[s,o]of Object.entries(t.entities)){if(o.device_id!==e)continue;const t=s.substring(0,s.indexOf(".")),n=wt[t];if(!n)continue;const r=xt(s);r&&(i[n][r]=s)}return i}(this.hass,t),this._deviceType=function(t){const e=new Set([...Object.keys(t.sensors),...Object.keys(t.binary_sensors),...Object.keys(t.buttons),...Object.keys(t.switches)]);return ft.some(t=>e.has(t))?"litter_box":pt.some(t=>e.has(t))?"feeder":gt.some(t=>e.has(t))?"fountain":"feeder"}(this._entities))}}render(){if(!this._config||!this.hass)return B``;if(!this._deviceId||!this._entities||!this._deviceType)return B`
        <ha-card>
      ...
      `;const t=this._config.entity||At(this.hass,this._deviceId)||"",e=this.hass.states[t]?.state;return"unavailable"===e?B`
        <ha-card>
          ${this._renderHeader()}
          <div class="unavailable">
    ...
    `}_renderHeader(){const t=this._config.name||(e=this.hass,i=this._deviceId,e.devices?.[i]?.name??void 0)||"PetLibro Device";var e,i;const s=this._config.entity||At(this.hass,this._deviceId)||"",o=function(t,e,i){const s=t.states[e]?.attributes?.entity_picture;return s||(t.devices?.[i]?.configuration_url??void 0)}(this.hass,s,this._deviceId),n=St(this.hass,this._entities.binary_sensors.online??this._entities.binary_sensors.wi_fi),r=this.hass.devices?.[this._deviceId]?.model;return B`
    ...
    `}_renderDeviceContent(){if(!this._entities)return F;switch(this._deviceType){case"feeder":return function(t,e,i,s){const o=Et(t,e.sensors.electric_quantity),n=St(t,e.binary_sensors.food_low),r=kt(t,e.sensors.today_feeding_quantity_weight),a=kt(t,e.sensors.today_feeding_times),c=Ct(t,e.sensors.last_feed_time),d=Ct(t,e.sensors.next_feed_time),l=kt(t,e.sensors.next_feed_quantity_weight),_=kt(t,e.sensors.feeding_plan_state),h=t.states[e.sensors.today_feeding_quantity_weight??""]?.attributes?.unit_of_measurement??"g",u=t.states[e.sensors.next_feed_quantity_weight??""]?.attributes?.unit_of_measurement??"g",v=St(t,e.binary_sensors.light_switch),m=kt(t,e.sensors.temperature),p=kt(t,e.selects.plate_position),g=kt(t,e.sensors.next_feeding_time),f=kt(t,e.sensors.next_feeding_end_time),b=kt(t,e.sensors.next_feeding_day),y=Et(t,e.sensors.remaining_cleaning_days);return B`
    <div class="metrics-grid">
      ${void 0!==o?B`
        <div class="metric-item">
          <ha-icon class="metric-icon" icon="${$=o,$>=90?"mdi:battery":$>=70?"mdi:battery-70":$>=50?"mdi:battery-50":$>=30?"mdi:battery-30":$>=10?"mdi:battery-10":"mdi:battery-alert"}"></ha-icon>
      ...
      <div class="metric-item ${n?"alert":""}">
        <ha-icon class="metric-icon" icon="${n?"mdi:bowl-mix-outline":"mdi:bowl-outline"}"></ha-icon>
        <div class="metric-content">
          <div class="metric-label">Food Status</div>
          <div class="metric-value">${n?"Low":"OK"}</div>
        </div>
      </div>
      ...
      ${void 0!==r?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${r} ${h}${a?` (${a}x)`:""}</div>
          </div>
        </div>
      `:F}
      ...
      ${d?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${d}${l?` (${l} ${u})`:""}</div>
          </div>
        </div>
      `:F}
      ...
      ${void 0!==m?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${m}°C</div>
          </div>
        </div>
      `:F}
    ...
    <div class="controls-row">
      ...
      ${""}
      ${e.switches.manual_feed_now?B`
        <button
          class="control-button ${St(t,e.switches.manual_feed_now)?"active":""}"
          @click=${()=>s(e.switches.manual_feed_now)}
        >
      ...
      ${e.buttons.rotate_food_bowl?B`
        <button class="control-button" @click=${()=>i(e.buttons.rotate_food_bowl)}>
          <ha-icon icon="mdi:rotate-3d-variant"></ha-icon>
          Rotate
        </button>
      `:F}

      ${e.buttons.ring_bell?B`
        <button class="control-button" @click=${()=>i(e.buttons.ring_bell)}>
      ...
      ${e.buttons.light_on||e.buttons.light_off?B`
        <button
          class="control-button ${v?"active":"secondary"}"
          @click=${()=>i(v?e.buttons.light_off:e.buttons.light_on)}
        >
          <ha-icon icon="mdi:lightbulb${v?"":"-outline"}"></ha-icon>
          Light
        </button>
      `:F}
    </div>
  `;var $}(this.hass,this._entities,t=>this._handleButtonPress(t),t=>this._handleSwitchToggle(t));case"fountain":return function(t,e,i){const s=Et(t,e.sensors.electric_quantity),o=Et(t,e.sensors.weight_percent),n=St(t,e.binary_sensors.water_low),r=kt(t,e.sensors.remaining_water),a=t.states[e.sensors.remaining_water??""]?.attributes?.unit_of_measurement??"mL",c=kt(t,e.sensors.today_drinking_amount),d=t.states[e.sensors.today_drinking_amount??""]?.attributes?.unit_of_measurement??"mL",l=kt(t,e.sensors.remaining_filter_days),_=kt(t,e.sensors.remaining_cleaning_days),h=St(t,e.binary_sensors.light_switch),u=void 0!==o?o<=10?"error":o<=25?"warning":"":"";return B`
    <div class="metrics-grid">
      ${void 0!==s?B`
        <div class="metric-item">
      ...
      <div class="metric-item ${n?"alert":""}">
        ...
        <div class="metric-content">
          ...
          ${void 0!==o?B`
            <div class="gauge-bar">
              <div class="gauge-fill ${u}" style="width: ${Math.min(100,o)}%"></div>
            </div>
          `:F}
        </div>
      </div>
      ...
      ${void 0!==r?B`
        <div class="metric-item">
          ...
          <div class="metric-content">
            ...
            <div class="metric-value">${Math.round(Number(r))} ${a}</div>
          </div>
        </div>
      `:F}
      ...
      ${void 0!==_?B`
        <div class="metric-item ${Number(_)<=1?"alert":""}">
          <ha-icon class="metric-icon" icon="mdi:broom"></ha-icon>
          <div class="metric-content">
            <div class="metric-label">Cleaning</div>
            <div class="metric-value">${_} days</div>
          </div>
        </div>
      `:F}
    </div>
    ...
    <div class="controls-row">
      ${e.buttons.light_on||e.buttons.light_off?B`
        <button
          ...
          @click=${()=>i(h?e.buttons.light_off:e.buttons.light_on)}
        >
  ...
  `;var v}(this.hass,this._entities,t=>this._handleButtonPress(t));case"litter_box":return function(t,e,i,s){const o=Et(t,e.sensors.electric_quantity),n=Et(t,e.sensors.weight_percent),r=St(t,e.binary_sensors.rubbish_full_state),a=kt(t,e.sensors.running_state),c=kt(t,e.sensors.remaining_cleaning_days),d=kt(t,e.sensors.deodorization_mode),l=St(t,e.switches.light_switch),_=St(t,e.switches.sound_switch),h=void 0!==n?n<=15?"error":n<=30?"warning":"":"";return B`
    <div class="metrics-grid">
      ${void 0!==o?B`
        <div class="metric-item">
          <ha-icon class="metric-icon" icon="${u=o,u>=90?"mdi:battery":u>=70?"mdi:battery-70":u>=50?"mdi:battery-50":u>=30?"mdi:battery-30":u>=10?"mdi:battery-10":"mdi:battery-alert"}"></ha-icon>
          <div class="metric-content">
            <div class="metric-label">Battery</div>
            <div class="metric-value">${Math.round(o)}%</div>
          </div>
    ...
    <div class="controls-row">
      ...
      ${e.switches.sound_switch?B`
        ...
        >
          <ha-icon icon="mdi:volume-${_?"high":"off"}"></ha-icon>
          Sound
        </button>
      `:F}
    </div>
  `;var u}(this.hass,this._entities,t=>this._handleButtonPress(t),t=>this._handleSwitchToggle(t));default:return B`<div class="unavailable">Unknown device type</div>`}}_handleButtonPress(t){this.hass.callService("button","press",{entity_id:t})}_handleSwitchToggle(t){this.hass.callService("switch","toggle",{entity_id:t})}_getDeviceTypeIcon(){switch(this._deviceType){case"feeder":return"mdi:food-drumstick";case"fountain":return"mdi:water";case"litter_box":return"mdi:cat";default:return"mdi:paw"}}_getAllEntityIds(){return this._entities?[...Object.values(this._entities.sensors),...Object.values(this._entities.binary_sensors),...Object.values(this._entities.buttons),...Object.values(this._entities.switches),...Object.values(this._entities.numbers),...Object.values(this._entities.selects)]:[]}};t([ht({attribute:!1})],Pt.prototype,"hass",void 0),t([ut()],Pt.prototype,"_config",void 0),t([ut()],Pt.prototype,"_deviceId",void 0),t([ut()],Pt.prototype,"_deviceType",void 0),t([ut()],Pt.prototype,"_entities",void 0),Pt=t([dt(vt)],Pt);let Ot=class extends at{static{this.styles=r`
    .editor-row {
      margin-bottom: 16px;
    }
    .editor-row label {
  ...
  `}setConfig(t){this._config=t}_getPetlibroDevices(){if(!this.hass?.entities||!this.hass?.devices)return[];const t=new Set;for(const e of Object.values(this.hass.entities)){const i=e;"petlibro"===i.platform&&i.device_id&&t.add(i.device_id)}const e=[];for(const i of t){const t=this.hass.devices[i],s=t?.name||t?.name_by_user||`Device ${i.slice(0,8)}`;e.push({id:i,name:s})}return e.sort((t,e)=>t.name.localeCompare(e.name))}render(){if(!this.hass||!this._config)return B``;const t=this._getPetlibroDevices();return B`
      <div class="editor-row">
        ...
        <select @change=${this._deviceChanged}>
          ...
          ${t.map(t=>B`
              <option value=${t.id} ?selected=${this._config.device_id===t.id}>
                ${t.name}
              </option>
            `)}
        </select>
        ${0===t.length?B`<div class="no-devices">
    ...
    `}_deviceChanged(t){const e=t.target.value;if(!e)return;const i={type:this._config.type,device_id:e,show_controls:this._config.show_controls};this._config.name&&(i.name=this._config.name),this._updateConfig(i)}_nameChanged(t){const e=t.target.value||void 0;this._updateConfig({...this._config,name:e})}_showControlsChanged(t){const e=t.target.checked;this._updateConfig({...this._config,show_controls:e})}_updateConfig(t){this._config=t;const e=new CustomEvent("config-changed",{detail:{config:t},bubbles:!0,composed:!0});this.dispatchEvent(e)}};t([ht({attribute:!1})],Ot.prototype,"hass",void 0),t([ut()],Ot.prototype,"_config",void 0),Ot=t([dt(mt)],Ot);var Tt=Object.freeze({__proto__:null,get PetlibroCardEditor(){return Ot}});export{Pt as PetlibroCard};
